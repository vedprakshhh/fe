...
const RecruiterApp: React.FC = () => {
  ...
  const fetchRecruiters = async () => {
    ...
    try {
      ...
      // Transform API data to match Recruiter interface
      const recruitersData = response.data.map((job: Recruiter) => ({
        id: job.id,
        name: job.name,
        email: job.email || 'N/A',
        phone: job.phone
  ...
  // Update a recruiter
  const updateRecruiter = async (id: number) => {
    setIsLoading(true);
    try {
  ...
  // Delete a recruiter
  const deleteRecruiter = async (id: number) => {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.recruitersSection}>
        {/* Modified header to ensure alignment */}
